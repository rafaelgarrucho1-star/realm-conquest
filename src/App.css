...
// Pontos da aldeia: por construção, níveis altos valem mais. Aldeia 100% = ~15.000
...
// MISSÕES — da CONTA do jogador (valem para todas as aldeias, não por aldeia)
const MISSIONS = [
  ...
  { id: 'm3', t: 'Treinar 50 tropas', d: 'Tenha 50 tropas no total (em casa ou em campo).', chk: (g) => Object.values(g.troops).reduce((a, b) => a + b, 0) >= 50, r: { w: 600, i: 600, h: 600 } },
  { id: 'm4', t: 'Erguer a Muralha (nível 5)', d: 'Suba a Muralha ao nível 5.', chk: (g) => g.levels.wall >= 5, r: { w: 1000, i: 500, h: 500 } },
  { id: 'm5', t: 'Ferreiro nível 5', d: 'Suba o Ferreiro ao nível 5 para liberar tropas.', chk: (g) => g.levels.smithy >= 5, r: { w: 1000, i: 1000, h: 800 } },
  { id: 'm6', t: 'Construir o Estábulo', d: 'Tenha um Estábulo para criar cavalaria.', chk: (g) => g.levels.stable >= 1, r: { w: 800, i: 800, h: 600 } },
...
const startState = () => ({
  ...
  research: {}, bonus: {}, isVip: true, questsClaimed: [],
...
function IsoBuilding({ id, cfg, level, hovered, selected, busy, onHover, onClick }) {
  ...
  // patch de terra PLANO (sem pedestal, pra não flutuar)
  const fp = w2 * 1.16, fh = fp / 2;
  ...
  const pillY = cy - H - roofH - (v.banner || v.flag ? 44 : v.spire ? 32 : 20);
  const pr = busy ? Math.max(0, Math.min(1, (busy.total - busy.left) / busy.total)) : 0;
  return (
    <g style={{ cursor: 'pointer', opacity: dim }} onClick={(e) => { e.stopPropagation(); onClick(id); }} onMouseEnter={() => onHover(id)} onMouseLeave={() => onHover(null)}>
      {/* sombra de contato suave (empilhada, colada no chão = fincado) */}
      <ellipse cx={cx} cy={cy + h2 * 0.86} rx={fp * 1.05} ry={fh * 0.82} fill="rgba(24,46,12,0.30)" />
      <ellipse cx={cx} cy={cy + h2 * 0.9} rx={fp * 0.78} ry={fh * 0.6} fill="rgba(24,46,12,0.28)" />
      {/* terra plana sob o prédio */}
      <polygon points={pts([pT, pR, pB, pL])} fill="#806035" />
      <polygon points={pts([pT, pR, pB, pL])} fill="none" stroke="#624a27" strokeWidth="1.5" />
      {glow && <polygon points={pts([[pT[0], pT[1] - 2], [pR[0] + 3, pR[1]], [pB[0], pB[1] + 2], [pL[0] - 3, pL[1]]])} fill="none" stroke="#f5e6a8" strokeWidth="3" />}
      {/* paredes */}
      ...
      {/* andaime quando em construção */}
      {busy?.type === 'build' && (<g><polygon points={pts([R, B, Bt, Rt])} fill="rgba(202,160,90,0.18)" /><line x1={R[0]} y1={R[1]} x2={Rt[0]} y2={Rt[1]} stroke="#caa15f" strokeWidth={2} /><line x1={B[0]} y1={B[1]} x2={Bt[0]} y2={Bt[1]} stroke="#caa15f" strokeWidth={2} /><line x1={(R[0] + B[0]) / 2} y1={(R[1] + B[1]) / 2} x2={(Rt[0] + Bt[0]) / 2} y2={(Rt[1] + Bt[1]) / 2} stroke="#caa15f" strokeWidth={1.5} /></g>)}
      {/* etiqueta: cronômetro (construção/recrutamento) ou nível */}
      <g transform={`translate(${cx}, ${pillY})`}>
        {busy ? (<>
          <rect x={-34} y={-15} width={68} height={22} rx={11} fill="rgba(20,14,8,0.93)" stroke={busy.type === 'build' ? '#caa15f' : '#d98a80'} strokeWidth="1.5" />
          <text x={0} y={1} textAnchor="middle" fontSize={11} fill="#f5e6a8" fontWeight="bold">{busy.type === 'build' ? '🔨' : '⚔️'} {fmtTime(busy.left)}</text>
          <rect x={-30} y={9} width={60} height={4} rx={2} fill="rgba(0,0,0,.55)" />
          <rect x={-30} y={9} width={60 * pr} height={4} rx={2} fill={busy.type === 'build' ? '#6db04f' : '#d0594a'} />
        </>) : level > 0 ? (<><circle r={13} fill="rgba(20,14,8,0.92)" stroke={glow ? '#f5e6a8' : '#caa15f'} strokeWidth={glow ? 2 : 1.2} /><text x={0} y={4} textAnchor="middle" fontSize={13} fill="#f5e6a8" fontWeight="bold">{level}</text></>)
...
function WallRing({ level, onGate, hovered }) {
  ...
  return (
    <g>
      ...
      <ellipse cx={cx} cy={cy + depth} rx={rx} ry={ry} fill="none" stroke="#5f5648" strokeWidth={thick} />
      {/* corpo da muralha */}
      <ellipse cx={cx} cy={cy} rx={rx} ry={ry} fill="none" stroke="#8a8273" strokeWidth={thick} />
      {/* topo iluminado */}
      <ellipse cx={cx} cy={cy - thick * 0.28} rx={rx} ry={ry} fill="none" stroke="#a39a87" strokeWidth={thick * 0.42} />
      {/* ameias discretas */}
      <ellipse cx={cx} cy={cy - thick * 0.42} rx={rx} ry={ry} fill="none" stroke="#736a59" strokeWidth={thick * 0.5} strokeDasharray="3 9" />
      {tw}
...
export default function RealmConquest() {
  ...
  const vipMult = isVip ? 1.2 : 1;
  const rates = { wood: production(L.woodcutter) * vipMult, iron: production(L.mine) * vipMult, wheat: production(L.farm) * vipMult };
  const points = villagePoints(L, g.conquered);
  const queueLimit = isVip ? 5 : 2;
  const busyOf = (id) => g.buildQueue[0]?.key === id
    ? { type: 'build', left: g.buildQueue[0].left, total: g.buildQueue[0].total }
    : (g.recruit[id] && g.recruit[id].length ? { type: 'train', left: g.recruit[id][0].left, total: g.recruit[id][0].unitTime } : null);
  ...
  const upgrade = (key) => {
    ...
    if (g.buildQueue.length >= queueLimit) { flash('⚠️ Fila cheia (' + queueLimit + ').'); return; }
  ...
  const claimQuest = (id) => {
    ...
    if ((g.questsClaimed || []).includes(id)) return;
    if (!m.chk(g)) { flash('🎯 Missão ainda não concluída.'); return; }
  ...
  const shared = { g, L, C, targets, popUsed, popMax, afford, upgrade, recruit, researchTroop, researchBonus, sendTroops, recall, setReport, setScreen, flash, rates, isVip, points, whCap, claimQuest };

  const NAV = [['village', '🏰', 'Aldeia'], ['mapa', '🗺️', 'Mapa'], ['missions', '🎯', 'Missões'], ['recrutar', '⚔️', 'Recrutar'], ['command', '🛡️', 'Tropas'], ['reports', '📜', 'Relatórios'], ['ranking', '🏆', 'Classificação'], ['tribo', '⚜️', 'Tribo'], ['amigos', '👥', 'Amigos'], ['perfil', '👤', 'Perfil']];

  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #ece0c6 0%, #dcc9a4 100%)', fontFamily: "'Cinzel', Georgia, serif", color: C.ink }}>
      ...
      <div style={{ background: `linear-gradient(${C.w2}, ${C.w1})`, borderBottom: `2px solid ${C.border}`, padding: '8px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', fontFamily: 'Georgia, serif' }}>
          ...
          <div style={{ background: 'rgba(0,0,0,.3)', border: `1px solid ${C.border}`, borderRadius: 8, padding: '4px 10px', fontSize: 12 }}><span style={{ color: C.sub }}>🏅 </span><b style={{ color: C.gold }}>{fmt(points)}</b><span style={{ color: C.sub }}> pts</span></div>
          ...
          <button onClick={() => setG(p => ({ ...p, isVip: !(p.isVip !== false) }))} title="Ativar/desativar VIP" style={{ background: isVip ? C.gold : 'transparent', border: `1px solid ${isVip ? C.gold : C.border}`, color: isVip ? '#1a1208' : C.sub, borderRadius: 8, padding: '4px 9px', cursor: 'pointer', fontWeight: 700, fontSize: 12, fontFamily: 'inherit' }}>👑 VIP</button>
      ...
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: 12 }}>
        ...
        {(g.buildQueue.length > 0 || Object.values(g.recruit).some(q => q.length) || g.movements.length > 0) && (
          <div style={{ background: `linear-gradient(${C.parch}, ${C.parchD})`, border: `2px solid ${C.border}`, borderRadius: 12, padding: 10, marginBottom: 10, fontFamily: 'Georgia, serif', boxShadow: '0 3px 10px rgba(0,0,0,.25)' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14 }}>
              <div style={{ flex: '1 1 240px', minWidth: 200 }}>
                <div style={{ fontSize: 12.5, fontWeight: 700, color: C.w1, marginBottom: 5 }}>🔨 Construção <span style={{ color: '#7a6a4a', fontWeight: 400 }}>({g.buildQueue.length}/{queueLimit}{isVip ? ' · 👑VIP' : ''})</span></div>
                {g.buildQueue.length === 0 && <div style={{ fontSize: 11.5, color: '#7a6a4a' }}>Nada em construção.</div>}
                {g.buildQueue.map((q, i) => { const pr = (q.total - q.left) / q.total; return (
                  <div key={i} style={{ marginBottom: 6 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.w1 }}><span>{BUILDINGS[q.key].icon} {BUILDINGS[q.key].name} <b>nv {q.level}</b>{i > 0 ? ' · aguardando' : ''}</span><span style={{ color: '#5a4426' }}>⏱ {fmtTime(q.left)}</span></div>
                    <div style={{ height: 7, background: '#cbb88f', borderRadius: 4, marginTop: 3, overflow: 'hidden' }}><div style={{ height: '100%', width: `${(i === 0 ? pr : 0) * 100}%`, background: `linear-gradient(90deg, ${C.greenB}, ${C.green})`, borderRadius: 4 }} /></div>
                  </div>); })}
              </div>
              <div style={{ flex: '1 1 240px', minWidth: 200 }}>
                <div style={{ fontSize: 12.5, fontWeight: 700, color: C.w1, marginBottom: 5 }}>⚔️ Recrutamento</div>
                {!Object.values(g.recruit).some(q => q.length) && <div style={{ fontSize: 11.5, color: '#7a6a4a' }}>Nada em treinamento.</div>}
                {Object.entries(g.recruit).map(([bk, q]) => { if (!q[0]) return null; const it = q[0]; const pr = (it.unitTime - it.left) / it.unitTime; const totLeft = it.left + (it.remaining - 1) * it.unitTime; return (
                  <div key={bk} style={{ marginBottom: 6 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.w1 }}><span>{TROOPS[it.key].icon} <b>{it.remaining}×</b> {TROOPS[it.key].name} <span style={{ color: '#7a6a4a' }}>· {BUILDINGS[bk].name}</span></span><span style={{ color: '#5a4426' }}>⏱ {fmtTime(totLeft)}</span></div>
                    <div style={{ height: 7, background: '#cbb88f', borderRadius: 4, marginTop: 3, overflow: 'hidden' }}><div style={{ height: '100%', width: `${pr * 100}%`, background: `linear-gradient(90deg, ${C.redB}, ${C.red})`, borderRadius: 4 }} /></div>
                  </div>); })}
              </div>
              {g.movements.length > 0 && (
                <div style={{ flex: '1 1 240px', minWidth: 200 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: C.w1, marginBottom: 5 }}>🧭 Tropas em campo</div>
                  {g.movements.slice(0, 5).map(m => (
                    <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.w1, marginBottom: 4 }}><span>{m.phase === 'stationed' ? '🛡️ apoiando' : m.mode === 'attack' ? (m.phase === 'back' ? '🏠 voltando de' : '⚔️ atacando') : '🛡️ apoio a'} {m.target.name}</span><span style={{ color: '#5a4426' }}>{m.phase !== 'stationed' ? '⏱ ' + fmtTime(m.left) : '—'}</span></div>
                  ))}
                </div>)}
            </div>
          </div>
        ...
        {screen === 'village' && (
          ...
          <div style={{ background: '#4a7a30', border: `3px solid ${C.w3}`, borderRadius: 14, overflow: 'hidden', boxShadow: '0 4px 14px rgba(0,0,0,.3)' }}>
            <svg viewBox="0 0 1000 680" style={{ width: '100%', display: 'block' }} onClick={() => { }}>
              ...
              <ellipse cx="330" cy="280" rx="130" ry="58" fill="rgba(255,255,255,0.05)" /><ellipse cx="660" cy="430" rx="150" ry="64" fill="rgba(0,0,0,0.06)" />
              <ellipse cx="600" cy="250" rx="70" ry="30" fill="rgba(110,168,74,0.5)" /><ellipse cx="380" cy="450" rx="80" ry="34" fill="rgba(63,106,42,0.4)" /><ellipse cx="720" cy="540" rx="60" ry="26" fill="rgba(63,106,42,0.4)" />
              ...
              {order.map(([id, cfg]) => (<IsoBuilding key={id} id={id} cfg={cfg} level={L[id]} hovered={hovered === id} selected={false} busy={busyOf(id)} onHover={setHovered} onClick={(bid) => setScreen(bid)} />))}
            </svg>
          </div>
          <div style={{ textAlign: 'center', fontSize: 11, color: '#6a5836', marginTop: 6, fontFamily: 'Georgia, serif' }}>👆 Clique em um edifício para abrir a página dele · clique no 🚪 portão para a Muralha · 🏰 torre central = suas tropas</div>
          </div>
          <VillageSidebar {...shared} />
          </div>
        )}
...
function MapScreen(p) {
  const { C, targets } = p;
  // jogadores simulados (no multiplayer real viriam do servidor)
  const OTHERS = [
    { name: 'Aldeia de Théo', x: 512, y: 332, kind: 'ally' }, { name: 'Bastião Real', x: 470, y: 366, kind: 'ally' },
    { name: 'Vila de Lúcia', x: 528, y: 372, kind: 'enemy' }, { name: 'Forte de Dante', x: 478, y: 318, kind: 'enemy' },
    { name: 'Refúgio Élfico', x: 462, y: 344, kind: 'player' }, { name: 'Aldeia de Nuno', x: 534, y: 356, kind: 'player' }, { name: 'Solar do Norte', x: 506, y: 320, kind: 'player' }
  ];
  const kindColor = { ally: '#3fa7c4', enemy: '#c0473c', player: '#caa53a', tribe: '#9a6ab0' };
  const colorOf = (t) => t.conquered ? '#5e8a3e' : t.diff === 'fraco' ? '#9aa0a6' : t.diff === 'médio' ? '#caa53a' : '#a0463e';
  const px = (wx) => 50 + (wx - HOME.x) * 2.3, py = (wy) => 50 + (wy - HOME.y) * 2.3;
  return (<Panel C={C} title="🗺️ Mapa do Mundo" sub="Sua aldeia ao centro · clique numa aldeia bárbara para atacar" onBack={() => p.setScreen('village')}>
    <div style={{ position: 'relative', width: '100%', paddingBottom: '66%', borderRadius: 10, border: `3px solid ${C.w3}`, overflow: 'hidden', background: '#6aa443' }}>
      {/* grama com leve textura + quadriculado */}
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 50% 45%, #7ab84e, #5b9237 70%, #4a7a2e)' }} />
      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(255,255,255,.05) 1px,transparent 1px),linear-gradient(90deg,rgba(0,0,0,.07) 1px,transparent 1px),linear-gradient(rgba(0,0,0,.07) 1px,transparent 1px)', backgroundSize: '100% 100%, 7.69% 7.69%, 7.69% 7.69%' }} />
      {/* florestas/relevo decorativo */}
      {[[12, 18], [85, 22], [20, 80], [78, 84], [60, 12], [38, 90]].map(([x, y], i) => <div key={i} style={{ position: 'absolute', left: `${x}%`, top: `${y}%`, fontSize: 18, opacity: .8, transform: 'translate(-50%,-50%)' }}>🌲</div>)}
      {/* sua aldeia */}
      <Village x={50} y={50} color="#3b82f6" label="Sua Capital (500|350)" me />
      {/* outros jogadores */}
      {OTHERS.map((o, i) => <Village key={'o' + i} x={px(o.x)} y={py(o.y)} color={kindColor[o.kind]} label={`${o.name} (${o.x}|${o.y})`} />)}
      {/* bárbaros (atacáveis) */}
      {targets.map(t => <Village key={t.id} x={px(t.x)} y={py(t.y)} color={colorOf(t)} barb label={`${t.conquered ? '✅ ' : '🪓 '}${t.name} (${t.x}|${t.y}) · ${t.owner}`} onClick={() => { p.setScreen('command'); p.flash('Abra 📤 Enviar e escolha o alvo pelas coordenadas.'); }} />)}
    </div>
    <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 10, fontSize: 12, fontFamily: 'Georgia, serif', color: C.w1 }}>
      <Legend c="#3b82f6" t="Você" /><Legend c="#caa53a" t="Outros jogadores" /><Legend c="#3fa7c4" t="Aliado" /><Legend c="#c0473c" t="Inimigo" /><Legend c="#9aa0a6" t="Bárbaro" /><Legend c="#5e8a3e" t="Conquistada" />
    </div>
    <div style={{ marginTop: 8, fontSize: 11, color: '#6a5836', fontFamily: 'Georgia, serif' }}>💡 As aldeias de jogadores aqui são simuladas. No multiplayer real, cada uma seria um jogador de verdade, com a cor da relação (sua tribo, aliado, inimigo).</div>
  </Panel>);
}
function Village({ x, y, color, label, me, barb, onClick }) {
  const [h, setH] = useState(false);
  return (<div onClick={onClick} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)} style={{ position: 'absolute', left: `${x}%`, top: `${y}%`, transform: 'translate(-50%,-50%)', cursor: onClick ? 'pointer' : 'default', zIndex: h ? 20 : me ? 5 : 1 }}>
    <svg width={me ? 30 : 24} height={me ? 30 : 24} viewBox="0 0 24 24" style={{ filter: `drop-shadow(0 0 ${h || me ? 5 : 2}px rgba(0,0,0,.5))` }}>
      <ellipse cx="12" cy="21" rx="8" ry="2.5" fill="rgba(0,0,0,.25)" />
      <rect x="5" y="11" width="14" height="9" rx="1" fill={color} stroke="rgba(0,0,0,.35)" strokeWidth="1" />
      <polygon points="4,11 12,4 20,11" fill={color} stroke="rgba(0,0,0,.35)" strokeWidth="1" />
      <rect x="10" y="14" width="4" height="6" fill="rgba(0,0,0,.4)" />
      {me && <circle cx="12" cy="3" r="2.5" fill="#fff" />}
      {barb && <text x="12" y="9.5" textAnchor="middle" fontSize="7">🪓</text>}
    </svg>
    {(h || me) && <div style={{ position: 'absolute', bottom: '120%', left: '50%', transform: 'translateX(-50%)', background: 'rgba(20,14,8,.96)', color: '#efe3c8', border: `1px solid ${color}`, borderRadius: 6, padding: '3px 8px', fontSize: 10.5, whiteSpace: 'nowrap', fontFamily: 'Georgia, serif', pointerEvents: 'none' }}>{label}</div>}
  </div>);
}
...
function RankingScreen(p) {
  const { C, g } = p;
  const myPts = villagePoints(g.levels, g.conquered);
...
/* ---------- BARRA LATERAL DA ALDEIA (estilo Tribal) ---------- */
function VillageSidebar(p) {
  const { C, L, g, rates, isVip, points, popUsed, popMax, claimQuest, setScreen } = p;
  const claimed = g.questsClaimed || [];
  const ready = MISSIONS.filter(m => !claimed.includes(m.id) && m.chk(g));
  const pending = MISSIONS.filter(m => !claimed.includes(m.id) && !m.chk(g));
  const show = [...ready, ...pending].slice(0, 3);
  const effects = [];
  if (isVip) effects.push(['👑', '+20% produção (VIP)']);
  if (L.wall > 0) effects.push(['🧱', `+${L.wall * 5}% defesa (muralha)`]);
  Object.entries(BONUS).forEach(([k, b]) => { if (g.bonus[k]) effects.push([b.icon, b.effect]); });
  return (
    <div style={{ flex: '1 1 250px', minWidth: 228, maxWidth: 330, display: 'flex', flexDirection: 'column', gap: 10 }}>
      <SidePanel C={C} title="📊 Resumo">
        <Line C={C} k="🏅 Pontos" v={fmt(points)} />
        <Line C={C} k="👥 População" v={`${fmt(popUsed)} / ${fmt(popMax)}`} />
        <Line C={C} k="🏰 Aldeias" v={fmt(1 + g.conquered)} />
      </SidePanel>
      <SidePanel C={C} title="⚒️ Produção por hora">
        <Line C={C} k="🪵 Madeira" v={fmt(rates.wood) + '/h'} />
        <Line C={C} k="⛏️ Ferro" v={fmt(rates.iron) + '/h'} />
        <Line C={C} k="🌾 Trigo" v={fmt(rates.wheat) + '/h'} />
      </SidePanel>
      <SidePanel C={C} title="✨ Efeitos ativos">
        {effects.length === 0 ? <div style={{ fontSize: 11.5, color: '#7a6a4a' }}>Nenhum efeito ativo.</div> : effects.map(([ic, t], i) => <div key={i} style={{ fontSize: 12, color: C.w1, marginBottom: 3 }}>{ic} {t}</div>)}
      </SidePanel>
      <SidePanel C={C} title={`🎯 Missões${ready.length ? ' · ' + ready.length + ' pronta(s)!' : ''}`}>
        {show.map(m => { const isReady = ready.includes(m); return (
          <div key={m.id} style={{ borderBottom: `1px solid ${C.parchD}`, padding: '5px 0' }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: C.w1 }}>{isReady ? '✅' : '⏳'} {m.t}</div>
            <div style={{ fontSize: 10.5, color: '#3d6a22' }}>🪵{fmt(m.r.w)} ⛏️{fmt(m.r.i)} 🌾{fmt(m.r.h)}</div>
            {isReady && <button onClick={() => claimQuest(m.id)} style={{ ...st.btn(C, 'green'), padding: 5, fontSize: 11, marginTop: 3 }}>🎁 Receber</button>}
          </div>); })}
        <button onClick={() => setScreen('missions')} style={{ ...st.btn(C, 'ghost'), padding: 6, fontSize: 11, marginTop: 7 }}>Ver todas →</button>
      </SidePanel>
    </div>
  );
}
function SidePanel({ C, title, children }) {
  return (<div style={{ background: `linear-gradient(${C.parch}, ${C.parchD})`, border: `2px solid ${C.border}`, borderRadius: 10, overflow: 'hidden', boxShadow: '0 3px 8px rgba(0,0,0,.22)' }}>
    <div style={{ background: `linear-gradient(${C.w2}, ${C.w1})`, color: C.gold, fontSize: 12.5, fontWeight: 700, padding: '6px 10px', letterSpacing: .3 }}>{title}</div>
    <div style={{ padding: 10, fontFamily: 'Georgia, serif' }}>{children}</div>
  </div>);
}
function Line({ C, k, v }) { return <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12.5, color: C.w1, padding: '2px 0' }}><span>{k}</span><b>{v}</b></div>; }

/* ---------- MISSÕES (tela completa) ---------- */
function MissionsScreen(p) {
  const { C, g, claimQuest } = p;
  const claimed = g.questsClaimed || [];
  return (<Panel C={C} title="🎯 Missões" sub="Recompensas em recursos por objetivos cumpridos" onBack={() => p.setScreen('village')}>
    <div style={{ background: 'rgba(63,113,134,.12)', border: `1px solid ${C.parchD}`, borderRadius: 8, padding: 10, marginBottom: 10, fontFamily: 'Georgia, serif', fontSize: 12, color: C.w1 }}>
      📌 As missões são da <b>sua conta</b> — valem para o jogador, <b>não por aldeia</b>. Cumpriu o objetivo, clique em <b>Receber</b> para ganhar os recursos.
    </div>
    {MISSIONS.map(m => { const done = claimed.includes(m.id); const ready = !done && m.chk(g); return (
      <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: done ? 'rgba(94,138,62,.15)' : '#fff', border: `1px solid ${done ? C.greenB : ready ? C.gold : C.parchD}`, borderRadius: 8, padding: 10, marginBottom: 6, fontFamily: 'Georgia, serif' }}>
        <div style={{ fontSize: 22 }}>{done ? '✅' : ready ? '🎁' : '⏳'}</div>
        <div style={{ flex: 1, color: C.w1 }}>
          <div style={{ fontWeight: 700, fontSize: 13 }}>{m.t}</div>
          <div style={{ fontSize: 11.5, color: '#7a6a4a' }}>{m.d}</div>
          <div style={{ fontSize: 11.5, color: '#3d6a22', marginTop: 2 }}>Prêmio: 🪵{fmt(m.r.w)} · ⛏️{fmt(m.r.i)} · 🌾{fmt(m.r.h)}</div>
        </div>
        {done ? <span style={{ color: C.green, fontWeight: 700, fontSize: 12 }}>Concluída</span>
          : ready ? <button onClick={() => claimQuest(m.id)} style={{ ...st.btn(C, 'green'), width: 'auto', padding: '8px 14px', fontSize: 12 }}>Receber</button>
            : <span style={{ color: '#a07a3a', fontSize: 11 }}>Em progresso</span>}
      </div>); })}
  </Panel>);
}
